...
import (
	...
	mo "go-application/common/models"
)
...
type SysOperaLog struct {
	...
	RequestMethod string    `json:"request_method" gorm:"type:varchar(128);comment:请求方式"`          // 请求方式
	...
	OperUrl       string    `json:"oper_url" gorm:"type:varchar(255);comment:访问地址"`                // 访问地址
	OperIp        string    `json:"oper_ip" gorm:"type:varchar(128);comment:客户端ip"`                // 客户端ip
	OperLocation  string    `json:"oper_location" gorm:"type:varchar(128);comment:访问位置"`           // 访问位置
	OperParam     string    `json:"oper_param" gorm:"type:varchar(255);comment:请求参数"`              // 请求参数
	Status        string    `json:"status" gorm:"type:varchar(4);comment:操作状态"`                    // 操作状态
	OperTime      time.Time `json:"oper_time" gorm:"type:timestamp;comment:操作时间"`                  // 操作时间
	JsonResult    string    `json:"json_result" gorm:"type:varchar(255);comment:返回数据"`             //
	Remark        string    `json:"remark" gorm:"type:varchar(255);comment:备注"`                    //
	LatencyTime   string    `json:"latency_time" gorm:"type:varchar(128);comment:耗时"`              // 耗时
	UserAgent     string    `json:"user_agent" gorm:"type:varchar(255);comment:ua"`                // ua
}
...
func (e *SysOperaLog) Generate() mo.ActiveRecord {
	o := *e
	return &o
}
